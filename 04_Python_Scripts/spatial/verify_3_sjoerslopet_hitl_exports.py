#!/usr/bin/env python3
"""
Verify 3-sjøersløpet O₁ gravel-road anchor HITL export — axis, GPS, PNG count.

Map-first FIT stream axis km 0–21.25 — not SUT_43 organiser GPX.

Usage (from repo root):
    python3 04_Python_Scripts/spatial/verify_3_sjoerslopet_hitl_exports.py
    python3 04_Python_Scripts/spatial/verify_3_sjoerslopet_hitl_exports.py --strict
"""
...
PANEL = _REPO / "03_Processed_Data" / "spatial" / "3_sjoerslopet_course" / "panel_1m.parquet"
TERRAIN_MAP = _REPO / "config" / "spatial_terrain_map_3_sjoerslopet.json"
OUT_DIR = _REPO / "06_Visualizations" / "3_sjoerslopet_hitl"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify 3-sjøersløpet HITL export")
    ...
    if not PANEL.exists():
        errors.append(
            ...
            "(config/spatial_align_manifest_3_sjoerslopet.json)"
        )
        for line in errors:
    ...
    print("=== 3-sjøersløpet HITL check (O₁ gravel-road anchor) ===")
    ...
    if not valid.any():
        ...
        if c_lat >= USKEDALEN_LAT_MIN:
            errors.append(f"centroid {c_lat:.4f}°N looks like Uskedalen — not Sandnes/Rogaland")
    ...
    if errors:
        return 1
    print("\nOK 3-sjøersløpet HITL verified.")
    return 0
