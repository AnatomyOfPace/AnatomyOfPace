#!/usr/bin/env python3
"""
Verify Gramstad Runde HITL PNG folder uses Sandnes/Gramstad FIT GPS.

Map-first local loop — not SUT_43 gramstad_band (km 29–41 organiser axis).

Usage (from repo root):
    python3 04_Python_Scripts/spatial/verify_gramstad_runde_hitl_exports.py
    python3 04_Python_Scripts/spatial/verify_gramstad_runde_hitl_exports.py --strict
"""
...
PANEL = _REPO / "03_Processed_Data" / "spatial" / "gramstad_runde_course" / "panel_1m.parquet"
TERRAIN_MAP = _REPO / "config" / "spatial_terrain_map_gramstad_runde.json"
OUT_DIR = _REPO / "06_Visualizations" / "gramstad_runde_hitl"
MANIFEST = OUT_DIR / "EXPORT_MANIFEST.json"

SANDNES_LAT_MIN = 58.75
SANDNES_LAT_MAX = 58.95
SANDNES_LON_MIN = 5.55
SANDNES_LON_MAX = 5.85
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify Gramstad Runde HITL export geography")
    ...
    if not PANEL.exists():
        errors.append(f"Panel missing: {PANEL} — run bootstrap_gramstad_runde_course.py")
        for line in errors:
    ...
    print("=== Gramstad Runde HITL geography check ===")
    print(f"  race_id: {race_id}")
    print(f"  axis:    {axis!r}")

    if axis.startswith("SUT_43"):
        errors.append("axis label is SUT_43 — check terrain map course_axis (use map-first stream)")
    ...
    if c_lat >= USKEDALEN_LAT_MIN:
        errors.append(
            f"panel centroid {c_lat:.4f}°N looks like Uskedalen — not Gramstad/Sandnes"
        )
    if not (SANDNES_LAT_MIN <= c_lat <= SANDNES_LAT_MAX):
        warnings.append(f"panel centroid lat {c_lat:.4f} outside expected Sandnes/Gramstad band")
    if not (SANDNES_LON_MIN <= c_lon <= SANDNES_LON_MAX):
        warnings.append(f"panel centroid lon {c_lon:.4f} outside expected Sandnes/Gramstad band")
    ...
    for idx, km_lo, km_hi in _chunk_windows(km_end):
        ...
        if clat >= USKEDALEN_LAT_MIN:
            bad_chunks.append(
                ...
                f"{clat:.4f}°N — Uskedalen (wrong course)"
            )

    if bad_chunks:
        errors.extend(bad_chunks)
    else:
        print(f"  OK all {len(_chunk_windows(km_end))} chunk GPS windows in Sandnes/Gramstad band")
    ...
    if MANIFEST.exists():
        ...
        errors.append(
            f"missing {MANIFEST.relative_to(_REPO)} — re-run export_hitl_chunks_gramstad_runde.sh"
        )
    ...
    if errors:
        ...
        print("  ./04_Python_Scripts/spatial/bootstrap_gramstad_runde_course.py --fit <your.fit>", file=sys.stderr)
        print("  ./04_Python_Scripts/spatial/export_hitl_chunks_gramstad_runde.sh", file=sys.stderr)
        return 1

    print("\nOK Gramstad Runde HITL geography verified.")
    return 0
