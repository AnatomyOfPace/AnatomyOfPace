#!/usr/bin/env python3
"""
Bootstrap Gramstad Runde Tier-0 course for map-first HITL (Gramstad, Sandnes).

Local training loop on FIT stream-distance axis — not SUT_43 gramstad_band
(km 29–41 organiser GPX sector).

1. Resolve canonical FIT (filename must contain Gramstad + rund/runde).
...
Usage (from repo root, after placing FIT locally):
    python3 04_Python_Scripts/spatial/bootstrap_gramstad_runde_course.py \\
...
MANIFEST = _REPO / "config" / "spatial_align_manifest_gramstad_runde.json"
TERRAIN_MAP = _REPO / "config" / "spatial_terrain_map_gramstad_runde.json"
DONOR_ID = "Subject_A"
RACE_ID = "gramstad_runde"
DONOR_DIR = _REPO / "02_Raw_Data" / "donors" / DONOR_ID


def _is_gramstad_runde_fit(path: Path) -> bool:
    name = path.name.lower()
    return "gramstad" in name and "rund" in name
...
def _discover_fit_candidates() -> list[Path]:
    ...
    for root in (_REPO / "02_Raw_Data", Path.home() / "Downloads", Path.home() / "Desktop"):
        ...
        for path in _iter_fit_files(root):
            if not _is_gramstad_runde_fit(path):
                continue
            key = str(path)
            if key in seen:
                continue
...
def _resolve_fit_path(explicit: Path | None) -> tuple[Path, str]:
    ...
    if len(discovered) > 1:
        ...
        raise FileNotFoundError(
            "Multiple Gramstad Runde FIT files found — pass exactly one with --fit:\n" + lines
        )

    raise FileNotFoundError(
        "No Gramstad Runde FIT found (filename must contain 'Gramstad' and 'rund').\n\n"
    ...
    activities[0].update(
        {
            ...
            "subject_id": DONOR_ID,
            "align_mode": "stream",
            "note": f"Gramstad Runde loop — FIT {activity_id} defines stream-distance course axis.",
        }
    )
...
def _patch_km_end(km_end: float, activity_id: str) -> None:
    ...
    corridor["notes"] = (
        "Tier 0 map-first local loop (Gramstad Runde, Sandnes). "
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Bootstrap Gramstad Runde map-first HITL course")
    parser.add_argument("--fit", type=Path, default=None, help="Path to Gramstad*rund*.fit")
    ...
    parser.add_argument("--discover", action="store_true", help="List Gramstad Runde FIT candidates")
    args = parser.parse_args()

    if args.discover:
        found = _discover_fit_candidates()
        if found:
            print("Gramstad Runde FIT matches:")
            for p in found:
                try:
            ...
            print("No filename containing both 'Gramstad' and 'rund' found.")
    ...
    print(
        "\nOK Gramstad Runde bootstrap complete.\n"
        ...
        "--terrain-map config/spatial_terrain_map_gramstad_runde.json add ...\n"
        "  PNGs:   ./04_Python_Scripts/spatial/export_hitl_chunks_gramstad_runde.sh"
    )
    return 0
