#!/usr/bin/env python3
"""
Verify Sunderunde training-loop O₁ gravel anchor HITL export — axis, GPS, PNG count.

Map-first FIT stream axis km 0–19.5 — low-intensity gravel/asphalt calibration.

Usage (from repo root):
    python3 04_Python_Scripts/spatial/verify_sunderunde_hitl_exports.py
...
PANEL = _REPO / "03_Processed_Data" / "spatial" / "sunderunde_training_loop" / "panel_1m.parquet"
TERRAIN_MAP = _REPO / "config" / "spatial_terrain_map_sunderunde.json"
OUT_DIR = _REPO / "06_Visualizations" / "sunderunde_hitl"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify Sunderunde HITL export")
    ...
    if not PANEL.exists():
        errors.append(
            ...
            "(config/spatial_align_manifest_sunderunde.json)"
        )
        for line in errors:
    ...
    print("=== Sunderunde HITL check (O₁ training gravel anchor) ===")
