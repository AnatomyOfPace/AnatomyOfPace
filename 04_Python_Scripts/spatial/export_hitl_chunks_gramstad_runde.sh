#!/usr/bin/env bash
# Bulk-export Gramstad Runde map-first HITL dashboards (1 km chunks).
...
MANIFEST="config/spatial_align_manifest_gramstad_runde.json"
...
if [[ ! -f "$PANEL" ]]; then
  echo "Panel missing: $PANEL — run bootstrap_gramstad_runde_course.py first" >&2
  exit 1
fi
...
act = (m.get("activities") or [{}])[0].get("activity_id", "Gramstad_Runde")
...
tmap = load_terrain_map(Path("config/spatial_terrain_map_gramstad_runde.json"))
panel = pd.read_parquet("03_Processed_Data/spatial/gramstad_runde_course/panel_1m.parquet")
...
PY
then
  echo "Preflight failed — run bootstrap_gramstad_runde_course.py first" >&2
  exit 1
fi
...
LOCOMOTION_SIDECAR="03_Processed_Data/spatial/gramstad_runde_course/locomotion_mode_1m.parquet"
...
manifest = {
    ...
    "geography": "Gramstad · Sandnes · Rogaland",
    "axis_label": "gramstad_runde stream km",
...
echo "OK Gramstad Runde HITL export complete → $OUT_DIR (${N_CHUNKS} chunks, km ${KM_START}–${KM_END})"
