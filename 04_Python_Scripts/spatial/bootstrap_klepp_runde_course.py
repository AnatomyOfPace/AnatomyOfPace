...
def main() -> int:
    ...
    print(
        ...
        "  Backup gold after labeling:\n"
        "    cp config/spatial_terrain_map_klepp_runde.json "
        "config/spatial_terrain_map_klepp_runde.gold_local.json\n"
        ...
        "--terrain-map config/spatial_terrain_map_klepp_runde.json ...\n"
        "  Train ML (after labeling):\n"
        "    python3 04_Python_Scripts/spatial/build_gold_training_set.py "
        "--terrain-map config/spatial_terrain_map_klepp_runde.json\n"
        "    python3 04_Python_Scripts/spatial/train_gold_suggester.py "
        "--training-set 03_Processed_Data/spatial/gold_training_set_klepp_runde.parquet "
        "--model-out 07_ML_Models/spatial/gold_suggester_klepp_runde_v0.joblib "
        "--metadata-out 07_ML_Models/spatial/gold_suggester_klepp_runde_v0_metadata.json "
        "--sector-id klepp_runde --no-source-holdout-eval\n"
        "  PNGs:   ./04_Python_Scripts/spatial/export_hitl_chunks_klepp_runde.sh "
        "(run AFTER train if you want ML predicted row)"
    )
    return 0
