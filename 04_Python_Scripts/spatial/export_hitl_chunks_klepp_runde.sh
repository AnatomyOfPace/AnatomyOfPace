#!/usr/bin/env bash
# Bulk-export Klepp Runde map-first HITL dashboards (1 km chunks).
...
MANIFEST="config/spatial_align_manifest_klepp_runde.json"
...
if [[ ! -f "$PANEL" ]]; then
  echo "Panel missing: $PANEL — run bootstrap_klepp_runde_course.py first" >&2
  exit 1
fi
...
act = (m.get("activities") or [{}])[0].get("activity_id", "Klepp_Runde")
...
tmap = load_terrain_map(Path("config/spatial_terrain_map_klepp_runde.json"))
panel = pd.read_parquet("03_Processed_Data/spatial/klepp_runde_course/panel_1m.parquet")
...
PY
then
  echo "Preflight failed — run bootstrap_klepp_runde_course.py first" >&2
  exit 1
fi
...
LOCOMOTION_SIDECAR="03_Processed_Data/spatial/klepp_runde_course/locomotion_mode_1m.parquet"
...
ML_MODEL="07_ML_Models/spatial/gold_suggester_klepp_runde_v0.joblib"
ML_PRED="03_Processed_Data/spatial/klepp_runde_course/klepp_runde_ml_predictions.parquet"
ML_ARGS=()
if [[ -f "$ML_MODEL" ]]; then
  ...
  echo "WARN no Klepp Runde ML model at $ML_MODEL — ML predicted strip will be empty" >&2
fi
...
echo "OK Klepp Runde HITL export complete → $OUT_DIR (${N_CHUNKS} chunks, km ${KM_START}–${KM_END})"
